...
class Camera_subscriber(Node):
    def __init__(self):
        super().__init__('camera_subscriber')
        self.package_share_dir = get_package_share_directory("robot_recognition")
        self.model_dir = os.path.join(self.package_share_dir, "scripts", "yolov8n.pt")  # Use YOLOv8n for faster inference
        ...
        self.subscription_depth = self.create_subscription(Image, 'camera/camera/depth/image_rect_raw', self.depth_camera_callback, 10)
    ...
    def camera_callback(self, data):
        ...
        try:
            ...
            resized_image = cv2.resize(cv_image, (320, 240))  # Reduce image size for faster processing
        ...
        self.get_logger().info(f'Profile results:\n{s.getvalue()}')
    ...
    # ... (keep other methods like deproject_pixel_to_point, publish_point, send_goal, etc.)
