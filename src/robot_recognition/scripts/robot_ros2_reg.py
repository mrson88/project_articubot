...
#                                 # self.send_goal(0.3762, 0.0,0.1210,0.0,0.8359,0.0,0.0,0)
#                                 self.send_goal(point_position.pose.position.x, point_position.pose.position.y,point_position.pose.position.z,0.8359,0.0,0.0,0)
...
class CameraSubscriber(Node):
    def __init__(self):
        ...
        self.action_client = ActionClient(self, ArticubotTask, 'test_server')
    ...
    def camera_callback(self, data):
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if inference_result.class_name in ["sports ball", "frisbee"]:
                    self.ball_detected = True
                    pixel_x = int((inference_result.left + inference_result.right) / 2)
                    pixel_y = int((inference_result.top + inference_result.bottom) / 2)
                    depth = self.depth_image[pixel_y, pixel_x]
                    
                    if depth > 0:
                        point_3d = self.pixel_to_3d(pixel_x, pixel_y, depth)
                        self.ball_pose = self.transform_to_base_link(point_3d)
                        self.get_logger().info(f'Ball detected at: x={self.ball_pose.position.x:.3f}, y={self.ball_pose.position.y:.3f}, z={self.ball_pose.position.z:.3f}')
                        
                        if self.findball and not self.action_client.server_is_ready():
                            self.send_pickup_goal()
    ...
    def send_pickup_goal(self):
        goal_msg = ArticubotTask.Goal()
        goal_msg.task = 5  # Assuming 5 is the task number for pickup
        goal_msg.p_x = self.ball_pose.position.x
        goal_msg.p_y = self.ball_pose.position.y
        goal_msg.p_z = self.ball_pose.position.z
        goal_msg.or_x = self.ball_pose.orientation.x
        goal_msg.or_y = self.ball_pose.orientation.y
        goal_msg.or_z = self.ball_pose.orientation.z
        goal_msg.or_w = self.ball_pose.orientation.w
