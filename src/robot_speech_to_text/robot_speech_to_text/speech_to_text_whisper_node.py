...
#         self.model, self.answer, self.history = faster_whisper.WhisperModel(model_size_or_path="medium", device='cpu'), "", []
...
class Speech_Whisper_Node(Node):
    def __init__(self):
        ...
        self.model = faster_whisper.WhisperModel(model_size_or_path="medium", device='cpu')
        self.answer = ""
        self.history = [
    ...
    def process_user_input(self):
        ...
        if "home" in self.user_text and self.talk_with_ai:
            ...
            # Use OpenAI-based chat instead of chromadb_response
            generator = self.openai_chat_response(self.user_text)
            print(f"Assistant: {generator}")
            self.play_text_to_speech(generator)
    ...
    def supress_callback(self, msg):
        """Set flag if wave input has to be discarded."""
        self.supress = msg.data
...
def main(args=None):
    ...
    node.run_chatbot()  # Call the new method to start the chatbot
    rclpy.shutdown()
